...
\begin{frame}{Who Am I?}
  \begin{itemize}
    \item Compiler researcher at Politecnico di Milano
  \end{itemize}
...
\begin{frame}{Slides are only so large, pretend you have hundreds of user actions}
    \centering
...
\begin{frame}{Systems grow in complexity}
    \begin{itemize}
        \item Domain experts often promise to never copy the state of interactive components, and then do it anyway. (Example: "In the car simulator, I want to copy the behaviour of a car, modify it, and after a while restore it to the previous behaviour")
        \item Often you want to isolate user actions in their own functions. 
    \end{itemize}

    \textbf{\textcolor{red}{If your solution cannot handle one of the previous points, you will have to rewrite the system.}}
...
% Q1
\node (q1) {Is your interactive\\component Turing-complete?};

% Yes -> Interpreter/JIT
\node[right=20mm of q1] (jit) {Interpreter / JIT};
\draw (q1) -- node[above]{Yes} (jit);

% No -> Q2
\node[below=9mm of q1] (q2) {Any reason now or in the future to\\copy/serialize state?};
\draw (q1) -- node[left]{No} (q2);

% Q2 No -> Coroutines/Threads
\node[below left=16mm and 2mm of q2] (coro) {Coroutines / Threads};
\draw (q2.west) -- ++(-10mm,0) |- node[pos=0.30, left]{No} (coro.north);

% Q2 Yes -> Q3
\node[below right=4mm and 6mm of q2] (q3) {Does it recursively call\\functions with user\\actions inside?};
\draw (q2.east) -- ++(10mm,0) |- node[pos=0.3, right]{Yes} (q3.north);

% Q3 No -> State machine + non-recursive
\node[below left=8mm and 6mm of q3] (smnonrec) {State machine\\+ non-recursive calls};
\draw (q3.west) -- ++(-10mm,0) |- node[pos=0.3, left]{No} (smnonrec.north);

% Q3 Yes -> State machine + recursive
\node[below=8mm of q3] (smrec) {State machine\\+ recursive calls};
\draw (q3.south) -- ++(00mm,0) |- node[pos=0.3, right]{Yes} (smrec.north);
...
\begin{frame}{Conclusions}
    \begin{itemize}
        ...
        \item The compiler knows what to do, but will not do it when you need serializable/copiable objects. All the complexities mentioned in this presentation would vanish if the compiler could do it.
        \item State machines of various complexity are the best tool you have to keep track of the complexity when it snowballs, while having guaranteed bounds on their cost, and guarantees on what is theoretically possible to implement.
    \end{itemize}
\end{frame}
...
\begin{frame}{Thanks!}
  ...
  Compleate Examples and slides: \href{https://shorturl.at/9DkZw}{https://shorturl.at/9DkZw}
\end{frame}
